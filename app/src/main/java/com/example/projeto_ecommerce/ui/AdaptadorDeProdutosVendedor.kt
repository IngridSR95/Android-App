...
import com.example.projeto_ecommerce.databinding.ItemProdutoVendedorBinding
import com.example.projeto_ecommerce.model.Produto

class AdaptadorDeProdutosVendedor(
    private val items: MutableList<Produto>,
    private val onEdit: (Produto) -> Unit,
    private val onDelete: (Produto) -> Unit
) : RecyclerView.Adapter<AdaptadorDeProdutosVendedor.VH>() {

    inner class VH(val b: ItemProdutoVendedorBinding) : RecyclerView.ViewHolder(b.root)

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        val b = ItemProdutoVendedorBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        return VH(b)
    }
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        if (imgId != 0) {
            holder.b.imgProduto.setImageResource(imgId)
        } else {
            holder.b.imgProduto.setImageResource(android.R.color.transparent)
        }
